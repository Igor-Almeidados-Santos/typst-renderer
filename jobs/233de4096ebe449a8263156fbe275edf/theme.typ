...
#let token-color(theme, kind, fallback) = {
  if kind == "keyword" {
    color-from(theme-value(theme, "keyword"), rgb(0xC5, 0x86, 0xC0))
  } else if kind == "string" {
    color-from(theme-value(theme, "string"), rgb(0xCE, 0x91, 0x78))
  } else if kind == "comment" {
    color-from(theme-value(theme, "comment"), rgb(0x6A, 0x99, 0x55))
  } else if kind == "number" {
    color-from(theme-value(theme, "number"), rgb(0xB5, 0xCE, 0xA8))
  } else if kind == "function" {
    color-from(theme-value(theme, "function"), rgb(0xDC, 0xDC, 0xAA))
  } else if kind == "operator" {
    color-from(theme-value(theme, "operator"), rgb(0xD4, 0xD4, 0xD4))
  } else if kind == "plain" {
    color-from(theme-value(theme, "plain"), fallback)
  } else {
    color-from(theme-value(theme, "text"), fallback)
  }
}

#let code-block(b, lang, content, segments: none, theme: (:)) = [
  ...
  #block(
    ...
    #block(
      ...
      #for (idx, line) in lines.enumerate() [
        ...
        #h(9pt)
        #if type(segments) == array and idx < segments.len() and type(segments.at(idx, default: none)) == array [
          #for seg in segments.at(idx, default: ()) [
            #let seg_text = seg.at("text", default: "")
            #let seg_kind = seg.at("kind", default: "plain")
            #let seg_fill = token-color(theme, seg_kind, code_ink)
            #text(font: b.font_mono, size: 9pt, fill: seg_fill)[#seg_text]
          ]
        ] else [
    ...
    if t == "heading" {
      ...
      if toc {
        ...
        code-block(
          ...
          segments: block.at("segments", default: none),
          theme: block_theme,
        )
      }
